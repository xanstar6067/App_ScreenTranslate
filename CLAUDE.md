...
- `overlay/OverlayRenderer.kt`: контраст, подбор шрифта, построение StaticLayout и отрисовка карточек.
- `overlay/LabelLayout.kt`: геометрия карточек — привязка к исходному тексту, рост только в свободное место. Чистая Kotlin-логика.
