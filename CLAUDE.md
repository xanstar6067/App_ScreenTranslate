...
Не заменять реальные тесты арифметическим шаблоном. Для pure Kotlin тестировать меню/абзацы/колонки/смешанные письменности/дубли, пакетирование, surrogate pairs, поиск плашки и геометрию наклонённых карточек. Для менеджера использовать поддельные TranslationProvider и TranslationStore. UI/OCR/MediaProjection/tap-through необходимо отдельно проверить на Android; успешная сборка не доказывает их работу на устройстве.
